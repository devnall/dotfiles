...
**`allowed_signers`:** Generated by bootstrap from `.gitconfig-user` (email + signingkey) so local `git log --show-signature` and `git verify-commit` validate our own SSH-signed commits — not just GitHub's Verified badge. Gitignored, regenerated per machine. If you change your signingkey, delete `config/git/allowed_signers` and re-run bootstrap.

**SSH key pin (multi-key 1Password agents):** The 1Password SSH agent serves *every* stored key, so a bare `Host *` config makes git offer the work key to GitHub and biometric-prompt for it before the personal key wins. The fix is host-scoped in `config/ssh/config`:

```sshconfig
Host github.com
  IdentitiesOnly yes
  IdentityFile ~/.dotfiles/config/git/personal_github.pub
```

This restricts the agent to one key *for github.com only* — other SSH hosts still use the full agent. Because this is a single GitHub account, the same auth key works for both work and personal repos; per-repo identity and commit signing are handled separately by `includeIf`/`user.signingkey`, so the transport key being "personal" doesn't affect work commit attribution. Bootstrap generates `personal_github.pub` from `.gitconfig-user`'s `signingkey` (the signing key string *is* the public key); it's gitignored and regenerated per machine, like `allowed_signers`. If you rotate the key, delete `config/git/personal_github.pub` and re-run bootstrap.
