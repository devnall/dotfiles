...
# Name
git config --global user.name "Drew Nall"
git config --global user.email "devnall@gmail.com"
...
git config --global alias.l3 "log --abbrev=7 --format='%C(bold cyan)%h%Creset %s %Cgreen(%cr) %C(blue)<%an>%Creset%C(yellow)%d%Creset'"
